...
import {Plus, X, Box, Square, Play} from 'lucide-react';
    ...
    return (
        <div className="flex flex-col h-screen w-full bg-white text-gray-800 font-sans overflow-hidden">
            ...
            <header className="h-12 border-b border-gray-200 flex items-center justify-between px-4 bg-white z-10 shrink-0">
                ...
                >
                    <button
                        className={`${running ? "bg-red-800 hover:bg-red-700" : "bg-green-800 hover:bg-green-700"} white rounded-md p-1`}
            ...
            <div className="flex-1 flex overflow-hidden relative">
                {/* Left Sidebar */}
                <div className={`bg-gray-50 border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out ${leftOpen ? 'w-80 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
                ...
                {/* Right Sidebar */}
                <div className={`bg-white border-l border-gray-200 flex flex-col transition-all duration-300 ease-in-out shadow-xl z-20 ${rightOpen ? 'w-80 translate-x-0' : 'w-0 translate-x-full overflow-hidden absolute right-0 h-full'}`}>
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <div className="flex-1 overflow-y-auto p-2">
                                {bodies.map(body => (
                                    <div
                                        key={body.name}
                                        onClick={() => { setEditingId(body.name); setRightSidebarView('edit'); }}
                                    ...
                                    >
                                        <div>
                                            ...
                                            <div className="text-[10px] text-gray-400 font-mono mt-0.5">Mass: {body.properties.mass}kg</div>
                                        </div>
                                        <div className="w-4 h-4 rounded-full border border-gray-200 shadow-sm" style={{ backgroundColor: body.color || '#3b82f6' }}></div>
                                    </div>
                                ))}
                            </div>
                            <div className="p-4 border-t border-gray-100 bg-gray-50">
                                <button onClick={() => setRightSidebarView('create')} className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-medium flex items-center justify-center gap-2 shadow-sm">
